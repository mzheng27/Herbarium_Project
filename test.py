...
#        image_list.append(cv2.imread(img_path, cv2.IMREAD_GRAYSCALE))
...
#        cudnn.benchmark = False
    ...
    for k, image_path in enumerate(image_list):
        ...
        for fname in os.listdir(result_folder):
            if ".txt" in fname and filename in fname:
                txt_file = open(os.path.join(result_folder, fname[:len(fname)-3]+"txt"),"r").read().split("\n")
                temptxt = txt_file[:-1]
        boxes = paragraphseg.expandBoxes(temptxt)
        count = len(boxes)
        newboxes = []
        for box in boxes:
            added = False
            for i, newb in enumerate(newboxes):
                overlap = paragraphseg.has_overlap(box,newb)
                if overlap != None:
                    newboxes[i] = overlap
                    added = True
            if not added:
